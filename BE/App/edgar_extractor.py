...
class EDGAR_Extractor(Edgar):
    ...
    def __init__(self):
        ...
        self.items_to_extract = (
            self.config["items_to_extract"] if self.config["items_to_extract"] else self.items_list
        )
        self.json_instance = {}
    ...
    def __is_json_key_present(self, json, key):
        try:
            buf = json[key]
        except KeyError:
            return False
    ...
    def __remove_html_tables(self, doc_10k: str, is_html: bool) -> str:
        """
        ...
        if is_html:
            ...
            for tbl in tables:
                tbl_text = EDGAR_Extractor.clean_text(EDGAR_Extractor.strip_html(str(tbl)))
    ...
    def __parse_item(
        ...
        item_section, positions = EDGAR_Extractor.get_item_section(
        ...
        if positions:
            ...
            if item_index in self.items_list and item_section == "":
                item_section = EDGAR_Extractor.get_last_item_section(
    ...
    def __extract_items(self, filing_metadata: Dict[str, Any]):
        """
        ...
        absolute_10k_filename = os.path.join(
            self.raw_filings_folder, filing_metadata["filename"]
        )
        ...
        doc_10k = self.__remove_html_tables(doc_10k, is_html=is_html)


        corp_name = filing_metadata['Company']
        report_num = filing_metadata['Date']

        if not self.__is_json_key_present(self.json_instance,corp_name):
            self.json_instance[corp_name] = {}
        
        if not self.__is_json_key_present(self.json_instance[corp_name],report_num):
            self.json_instance[corp_name][report_num] = {}



        # Extract the text from the document and clean it
        text = EDGAR_Extractor.strip_html(str(doc_10k))
        text = EDGAR_Extractor.clean_text(text)
        ...
        for i, item_index in enumerate(self.items_list):
            ...
            item_section, positions = self.__parse_item(
                text, item_index, next_item_list, positions
            )

            # Remove multiple lines from the item section
            item_section = EDGAR_Extractor.remove_multiple_lines(item_section)
    ...
    def doExtractFromRAWs(self):
        ...
        for report in list_of_series :
            ...
            with open(absolute_json_filename, "w", encoding='UTF-8') as filepath:
                d = {}
                d[report_company] = self.json_instance[report_company]
