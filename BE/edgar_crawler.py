...
class EDGAR_Crawler(Edgar):
	'''
	This is a class which crawl and download SEC filings (HTML)
	...
	def __init__(self):
		...
		if not os.path.isfile(os.path.join(self.DATASET_DIR, 'companies_info.json')):
			with open(os.path.join(self.DATASET_DIR, 'companies_info.json'), 'w') as f:
				json.dump(obj={}, fp=f)

	def __retryRequestsSession(
		self,
		retries: int = 5,
			...
			for year in range(start_year, end_year + 1):
				for quarter in quarters:
					...
					with tempfile.TemporaryFile(mode="w+b") as tmp:
						...
						try: 
							request = self.__retryRequestsSession(
	...
	def __crawlFromIndices(
		...
		with open(os.path.join(self.DATASET_DIR, 'companies_info.json')) as f:
	...
	def doCrawl(self):
		'''
		Do crawl from HTML filings and store them.

		`config.json` decides saving paths, which companies to crawl, etc.
		see `config.json` file or check out README.md
		'''
		self.__downloadIndices(		
			start_year=self.config['start_year'],
			end_year=self.config['end_year'],
			quarters=self.config['quarters'],
			skip_present_indices=self.config['skip_present_indices'],
			indices_folder=self.indices_folder,
		...
		for year in range(self.config['start_year'], self.config['end_year'] + 1):
			for quarter in self.config['quarters']:
				filepath = os.path.join(self.indices_folder, f'{year}_QTR{quarter}.tsv')

				if os.path.isfile(filepath):
					tsv_filenames.append(filepath)

		df = self.__getIndicesInNeed(
			tsv_filenames=tsv_filenames,
			filing_types=self.config['filing_types'],
			cik_tickers=self.config['cik_tickers'],
			user_agent=self.config['user_agent']
		)
		...
		if os.path.exists(self.filings_metadata_filepath):
			...
			for _, series in pd.read_csv(self.filings_metadata_filepath, dtype=str).iterrows():
				if os.path.exists(os.path.join(self.raw_filings_folder, series['filename'])):
		...
		for series in tqdm(list_of_series, ncols=100):
			...
			series = self.__crawlFromIndices(
				series=series,
				filing_types=self.config['filing_types'],
				raw_filings_folder=self.raw_filings_folder,
				user_agent=self.config['user_agent']
			)
			...
			if series is not None:
				...
				final_df.to_csv(self.filings_metadata_filepath, index=False, header=True)

		print(f'\nFilings metadata exported to {self.filings_metadata_filepath}')
